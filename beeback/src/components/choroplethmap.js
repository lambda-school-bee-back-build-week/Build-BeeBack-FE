...
class ChoroplethMap extends React.Component {
  ...
  componentDidMount() {
    axios
      .get("https://beeback.herokuapp.com/api/neonic2/2016")
      .then(res => this.setState({ data: [...res.data] }))

      .catch(err => console.error(err));

    axios
      .get("https://beeback.herokuapp.com/api/state-charting")
      .then(res => this.setState({ state_charting: res.data }))
      .catch(err => console.error(err));
  }
  if;
  render() {
          ...
          <PlotlyComponent
            onClick={data => {
              let chart = this.state.state_charting[
                us_state_dict[data.points[0].text]
              ];
              if (chart !== undefined) {
